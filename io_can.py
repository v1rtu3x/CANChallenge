...
def get_bus():
    """Return a global python-can bus instance."""
    global _bus
    if _bus is None:
        _bus = can.interface.Bus(channel=VCAN_INTERFACE, bustype="socketcan")
    return _bus
...
def setup_vcan():
    ...
    try:
        ...
        result = subprocess.run(
            f"ip link show {VCAN_INTERFACE}", shell=True, capture_output=True
        )
        if result.returncode != 0:
            subprocess.run(
                f"ip link add dev {VCAN_INTERFACE} type vcan", shell=True, check=True
            )
...
def start_cangen():
    ...
    try:
        ...
        print(
            f"[INFO] Started cangen to generate background CAN traffic on {VCAN_INTERFACE}"
        )
        return True
...
def send_can_frame(arb_id, data):
    ...
    try:
        msg = can.Message(
            arbitration_id=arb_id, data=bytearray(data), is_extended_id=False
        )
        get_bus().send(msg)
        print(f"[SENT] ID=0x{arb_id:03X}, Data={[hex(b) for b in data]}")
        return True
